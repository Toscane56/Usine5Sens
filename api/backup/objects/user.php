...
//-----METHODES PUBLIQUES
    ...
    public function create() {
        ...
        $default_values = array("email_n" => 1, "push" => 1, "token" => $this->_generate_token(true));
    ...
    public function get_token_by_pseudo_and_password() {
        ...
        $request = "SELECT token from $this->table_name WHERE email=:email AND password=:password";
        ...
        if (! ($stmt->bindParam(":email", $this->properties["email"]) and $stmt->bindParam(":password", $h_passwd))) {
            return errors("User_bindParam", $stmt->errorInfo()[2]);
        }
        ...
        if ($stmt->rowCount() != 1) {
            return errors("User_count", "L'email [" . $this->properties["email"] . "] n'existe pas ou est déjà utilisé");
        }
